...
from hermes_cli.commands import (
    COMMAND_REGISTRY, _is_gateway_available, _is_gateway_menu_advertised,
    _iter_plugin_command_entries, _resolve_config_gates)
...
def telegram_bot_commands(*, include_plugins: bool = True) -> list[tuple[str, str]]:
    ...
    pairs = [(cmd.name, cmd.description) for cmd in _gateway_menu_commands()]
    if include_plugins:
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    """(slash_name, description, usage_hint) triples for Slack: every menu-advertised command
    ...
    available = _gateway_menu_commands()
