...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "_turn_generation": 0,
            "_api_recovery_marker": None,
            "_hosted_room_task": None,
            "inflight_turn": None,
            "created_at": now,
            "last_active": now,
            "running": False,
...
def _run_prompt_submit(
    ...
    *,
    ...
    with session["history_lock"]:
        ...
        agent = session["agent"]
        turn_generation = int(session.get("_turn_generation", 0) or 0) + 1
        session["_turn_generation"] = turn_generation
        # A new turn supersedes any in-memory recovery ownership from an older
        # generation.  The durable marker remains authoritative until this
        # turn's own terminal state proves it can retire it.
        session["_api_recovery_marker"] = None
        hosted_task = session.get("_hosted_room_task")
        hosted_task_identity = (
            tuple(
                hosted_task.get(field)
                for field in (
                    "room_id",
                    "task_id",
                    "thread_id",
    ...
    def run():
        ...
        def _current_hosted_task_identity() -> tuple[Any, ...] | None:
        ...
        def _hosted_task_still_owned() -> bool:
            try:
                with session["history_lock"]:
                    if int(session.get("_turn_generation", 0) or 0) != turn_generation:
                        return False
                    return _current_hosted_task_identity() == hosted_task_identity
            except Exception:
                return False

        def _marker_belongs_to_turn() -> bool:
            keys = dict.fromkeys(
                (
                    marker_key,
                    api_recovery_marker_key,
            ...
            for key in keys:
                if not key:
                    continue
                marker = read_turn_marker(marker_home, key)
                if marker is None:
                    continue
                saw_marker = True
                if marker.get("attempts") != marker_attempt:
                    return False
                if (marker.get("resume_reason") or None) != expected_reason:
                    return False
            # A failed marker write is already unrecoverable on disk; there is
            # nothing safe for this turn to retire in that case.
            return True if not saw_marker else True

        def _api_recovery_marker_is_current() -> bool:
            if not api_recovery_auto_continue or not api_recovery_marker_key:
                return False
            with session["history_lock"]:
                ...
                if not isinstance(state, dict):
                    return False
                if (
                    state.get("turn_generation") != turn_generation
                    or state.get("marker_key") != api_recovery_marker_key
                    or state.get("attempts") != marker_attempt
                ):
                    return False
            marker = read_turn_marker(marker_home, api_recovery_marker_key)
            return bool(
                marker is not None
                and marker.get("attempts") == marker_attempt
                and marker.get("resume_reason")
                == API_OUTAGE_RECOVERY_RESUME_REASON
            )

        def _terminal_state_can_retire() -> bool:
            if terminal_callback is not None and not terminal_receipt_committed:
                return False
            if not _hosted_task_still_owned():
                return False
            if _api_recovery_marker_is_current():
                return False
            return _marker_belongs_to_turn()

        def _retire_terminal_state() -> bool:
            if not _terminal_state_can_retire():
                return False
            ...
            with session["history_lock"]:
                ...
                if (
                    ...
                    and hosted_task_identity is not None
                    and _current_hosted_task_identity() == hosted_task_identity
                ):
                    session.pop("_hosted_room_task", None)
                state = session.get("_api_recovery_marker")
                if isinstance(state, dict) and state.get("turn_generation") == turn_generation:
        ...
        def _attempt_terminal_receipt(
            ...
            nonlocal terminal_receipt_attempted, terminal_receipt_committed
            if terminal_callback is None:
                terminal_receipt_committed = True
                return True
            if terminal_receipt_attempted:
                return terminal_receipt_committed
            terminal_receipt_attempted = True
            # A callback is an external hosted-room write.  Do not allow a
            # late callback from an older turn to settle a newer task.
            if not _hosted_task_still_owned():
                return False
            receipt = {
                "status": status,
        ...
        try:
            ...
            if api_recovery_auto_continue:
                ...
                with session["history_lock"]:
                    session["_api_recovery_marker"] = {
                        ...
                        "marker_key": api_recovery_marker_key,
                        "attempts": marker_attempt,
                    }
            ...
            _attempt_terminal_receipt("failed", "", str(e))
            try:
                # Close the turn with the same terminal error frame shape as
                # the returned-error path (uniform client handling), retaining
                # the failed turn for resume replay.  Marker retirement is
                # owned by the same terminal state machine as the success path.
                _emit_terminal_turn_error(
                    sid,
                    session,
                    e,
                    retire_marker=_terminal_state_can_retire(),
                )
                turn_error_retained = True
            ...
            if terminal_receipt_committed and not _api_recovery_marker_is_current():
                _retire_terminal_state()
