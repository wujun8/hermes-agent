...
def _dispatch_manual_compression(
    ...
    """Run one manual compression after its exact session-control lease is held."""
    ...
    try:
        ...
        after_tokens = (
            estimate_request_tokens_rough(
                after_messages,
                system_prompt=system_prompt_after,
                tools=tools_after,
            )
            if after_count
            else 0
        ...
        return _ok(
            rid,
            {
                ...
                "output": "\n".join(
                    filter(
                        None,
                        [summary["headline"], summary["token_line"], summary.get("note")],
                    )
                ),
            },
        )
    except CompressionLockHeld as exc:
        # Lock-skip is a clean no-op, not a failed command.
        from agent.manual_compression_feedback import describe_compression_lock_skip

        return _ok(
            rid,
            {"type": "exec", "output": describe_compression_lock_skip(exc.holder)},
        )
    except Exception as exc:
        finalize_context_engine_compression_notification(
            session["agent"],
...
def _(rid, params: dict) -> dict:
    ...
    sid = params.get("session_id", "")
    if base == "micro":
        # Claim the live session before any config/DB/runtime work.  Prompt and
        # session transitions check the same token, so releasing history_lock
        # does not reintroduce a check-then-act race.
        from hermes_cli.commands import resolve_command

        micro_def = resolve_command(base)

        def _run_micro_slash():
            if micro_def is not None and micro_def.busy_policy == "reject":
                micro_target, micro_claim = _claim_live_micro_control(sid, session)
            else:
                micro_target, micro_claim = None, None
            if micro_claim is _MICRO_TURN_BUSY:
                return _err(rid, 4009, _MICRO_TURN_BUSY_MESSAGE)
            if micro_claim is _SESSION_CONTROL_BUSY:
                return _err(rid, 4009, _MICRO_CONTROL_BUSY_MESSAGE)
            if isinstance(micro_claim, str):
                return micro_claim
            if micro_target is None:
                # Cold sessions keep the worker fallback, which owns row
                # creation and later policy hydration.
                return _live_slash_command_output(sid, session, base, arg)

            micro_response = None
            micro_release = None
            try:
                micro_response = _live_slash_command_output(
                    sid,
                    session,
                    base,
                    arg,
                ...
                micro_release = _release_live_micro_control(micro_target)
            release_warning = str(getattr(micro_release, "warning", "") or "")
            if release_warning:
                if micro_response is None:
                    micro_response = release_warning
                elif release_warning not in str(micro_response):
                    micro_response = f"{micro_response}\n\n{release_warning}"
            if not micro_release:
                if release_warning:
