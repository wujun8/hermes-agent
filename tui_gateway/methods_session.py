...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        def _reuse_live_payload(
            sid: str,
            session: dict,
            *,
            rebind_transport: bool = True,
        ) -> dict | None:
        ...
        def _reuse_live_response(sid: str, session: dict) -> dict:
            ...
            with _session_resume_lock:
                ...
                # Rebind while the resume lock still excludes the orphan reaper,
                # but force the projection onto the omit-messages path so no
                # profile DB read blocks every unrelated resume/close RPC.
