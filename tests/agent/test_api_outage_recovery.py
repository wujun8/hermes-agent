...
def test_stale_circuit_parks_without_fallback_resets_streak_before_same_boundary_retry(
    monkeypatch,
):
    ...
    waiter = MagicMock()
    waiter.wait.return_value = "recovered"
    agent._api_outage_recovery_waiter = waiter
    stale_error = RuntimeError(
        "Provider has been unresponsive (no response received) for "
        "5 consecutive stale attempts — aborting this call to "
    ...
    def transport(_kwargs):
        snapshots.append([dict(message) for message in agent._session_messages])
        if len(snapshots) == 1:
            raise stale_error
    ...
    result = _run(agent, transport, conversation_history=completed_tool_history)
