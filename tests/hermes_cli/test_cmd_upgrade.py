...
def test_cmd_upgrade_resets_release_and_reapplies_local_patches(capsys):
    """Release upgrades reset to the tag, then replay $HERMES_HOME/local-patches."""
    ...
    def fake_run(cmd, **kwargs):
        ...
        if "status --porcelain" in joined or "ls-files --unmerged" in joined:
            return _git_completed(cmd)
        if "reset --hard v1.2.3" in joined:
            return _git_completed(cmd)
        return _git_completed(cmd)
    ...
    with patch("shutil.which", return_value=None), \
         ...
         patch("hermes_cli.update_cmd._list_local_release_patches", return_value=[]), \
         patch("subprocess.run", side_effect=fake_run):
    ...
    assert any("reset --hard v1.2.3" in command for command in commands)
    assert "git merge --no-edit v1.2.3" not in commands
