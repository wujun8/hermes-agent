...
class TestMountedDockerTrustContextPrecedence:
    ...
    def test_context_policy_still_decides_for_all_terminal_guards(
        ...
        if context == "cron":
            ...
            monkeypatch.setattr(A.approval_context, "_get_cron_approval_mode", lambda: mode)
        elif context == "single_query":
            monkeypatch.setenv("HERMES_SINGLE_QUERY_SESSION", "1")
            monkeypatch.setenv("HERMES_INTERACTIVE", "1")
            monkeypatch.setattr(
                A.approval_context, "_get_single_query_approval_mode", lambda: mode
            )
        else:
            monkeypatch.setenv("HERMES_SESSION_PLATFORM", "webhook")
            monkeypatch.setattr(
                A.approval_context, "_get_unattended_approval_mode", lambda: mode
            )
    ...
    def test_legacy_direct_guard_keeps_runtime_floors_before_trust(
        ...
        self._enable_trust(monkeypatch)
        monkeypatch.setattr(A.approval_context, "_get_approval_config", lambda: config)
