    ...
    *,
    ...
    if callable(setter):
        try:
            ...
            logger.warning("Micro-compaction engine capability update failed: %s", exc)
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    raw_show_retry_status = _agent_section.get("show_retry_status", False)
    agent._show_retry_status = str(raw_show_retry_status).strip().lower() in {
        "true", "1", "yes", "on",
    }
...
def _build_context_engine(agent, _agent_cfg, cs, _custom_providers, _effective_context_length, session_db):
    ...
    agent._micro_compact_global_value = cs.micro_compact
